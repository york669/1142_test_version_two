...
class Record():
    ...
    ----------
    G_spread :
        保留原介面。此版本暫不將 spread 納入績效公式，
        因為原始程式對 spread 的單位定義不一致。
        若你之後要納入滑價 / bid-ask spread，可再擴充。

    G_tax :
        股票: 賣出時交易稅率，例如 0.003
        期貨: 進出場各收一次交易稅率，例如 0.00002

    G_commission :
        股票: 手續費率，例如 0.001425
        期貨: 每邊固定手續費(TWD)，例如 TXF = 20, MXF = 12.5
    ...
    def __init__(self, G_spread=3.628e-4, G_tax=0.00002, G_commission=4.411e-5, isFuture=True):
        ## 期貨還是股票
        self.isFuture = isFuture
        ...
        self.Profit = []
        self.Profit_rate = []
        self.Capital_rate = []   # 以 1 為本金之歷次完整交易本利和清單
    ...
    def _get_multiplier(self, product):
        ...
        if product == 'TXF':
            ...
            return 50
        elif product == 'TMF':   # 微型台指
            return 10
        else:
            # 其他期貨商品若未特別指定，先用 1
            # 若你之後有更多商品，可在這裡補上正確乘數
            return 1

    def _calc_stock_long(self, entry_price, exit_price):
        """
        股票作多：
        進場買入 => 付出 entry_price * (1 + commission)
        出場賣出 => 收回 exit_price * (1 - commission - tax)
        """
        entry_cost = entry_price * (1 + self.G_commission)
        exit_value = exit_price * (1 - self.G_commission - self.G_tax)

        profit = exit_value - entry_cost
        profit_rate = profit / entry_cost
        capital_rate = 1 + profit_rate

        return profit, profit_rate, capital_rate

    def _calc_stock_short(self, entry_price, exit_price):
        """
        股票作空：
        進場放空(賣出) => 收入 entry_price * (1 - commission - tax)
        出場回補(買入) => 支出 exit_price * (1 + commission)

        這裡沿用「以進場名目金額為分母」來定義報酬率。
        """
        entry_value = entry_price * (1 - self.G_commission - self.G_tax)
        cover_cost = exit_price * (1 + self.G_commission)

        profit = entry_value - cover_cost

        # 為避免極端情況 denominator <= 0
        denominator = abs(entry_price) if abs(entry_price) > 1e-12 else 1e-12
        profit_rate = profit / denominator
        capital_rate = 1 + profit_rate

        return profit, profit_rate, capital_rate

    def _calc_future_long(self, product, entry_price, exit_price):
        """
        期貨作多：
        淨利 = 價差損益 - 進場手續費 - 出場手續費 - 進場稅 - 出場稅
        """
        multiplier = self._get_multiplier(product)

        gross_pnl = (exit_price - entry_price) * multiplier

        entry_commission = self.G_commission
        exit_commission = self.G_commission

        entry_tax = entry_price * multiplier * self.G_tax
        exit_tax = exit_price * multiplier * self.G_tax

        total_cost = entry_commission + exit_commission + entry_tax + exit_tax
        profit = gross_pnl - total_cost

        # 這裡用「進場名目價值 + 進場成本」作為報酬率分母
        # 若你要改成以保證金為分母，之後再改這裡即可
        denominator = entry_price * multiplier + entry_commission + entry_tax
    ...
    def _calc_future_short(self, product, entry_price, exit_price):
        """
        期貨作空：
        淨利 = (進場價 - 出場價) * multiplier - 各項成本
        """
        multiplier = self._get_multiplier(product)
    ...
    def Cover(self, BS, Product, CoverTime, CoverPrice, CoverQty):
        ...
        if BS == 'S' or BS == 'Sell':
            for _ in range(CoverQty):
                ...
                entry_price = TmpInterest[3]
                exit_price = CoverPrice
                trade_product = TmpInterest[1]

                if self.isFuture:
                    profit, profit_rate, capital_rate = self._calc_future_long(
                        trade_product, entry_price, exit_price
                    )
                else:
                    profit, profit_rate, capital_rate = self._calc_stock_long(
                        entry_price, exit_price
                    )

                self.Profit.append(profit)
                self.Profit_rate.append(profit_rate)
                self.Capital_rate.append(capital_rate)
        ...
        # --------------------------------
            ...
            for _ in range(CoverQty):
                ...
                if self.isFuture:
                    profit, profit_rate, capital_rate = self._calc_future_short(
                        trade_product, entry_price, exit_price
                    )
                else:
                    profit, profit_rate, capital_rate = self._calc_stock_short(
                        entry_price, exit_price
                    )
