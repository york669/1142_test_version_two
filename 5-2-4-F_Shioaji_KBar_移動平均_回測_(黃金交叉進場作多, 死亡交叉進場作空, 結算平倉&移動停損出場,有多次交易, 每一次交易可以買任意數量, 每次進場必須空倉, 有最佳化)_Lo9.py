...
def optimizeMA(OrderRecord,KBar_dic,period_range_Long, period_range_Short, MoveStopLoss=10, Order_Quantity=1, isFuture='False', G_commission=0.001425):
    ...
    bestcapital=-1000000  ## 最好的 "最後的本利和" (以 1 進場)
    bestcapital_series=[]
    bestperiodLong=0
    bestperiodShort=0
...
### 產出歷次盈虧(TWD,點數)交易績效圖(畫出 "歷次盈虧(TWD,點數)清單")
...
## 選擇參數: LongMAPeriod=10, ShortMAPeriod=2
LongMAPeriod = input("請輸入MA 長週期: ") 
ShortMAPeriod = input("請輸入MA 短週期: ")
w1 = float(input("商品1 投資權重: "))
w2 = float(input("商品2 投資權重: "))
CumulativeCapitalRate_series_1, final_return_1 = back_test(OrderRecord_1, KBar_dic_TXF, LongMAPeriod=LongMAPeriod, ShortMAPeriod=ShortMAPeriod, MoveStopLoss=10, Order_Quantity=1)
CumulativeCapitalRate_series_2, final_return_2 = back_test(OrderRecord_2, KBar_dic_TXF, LongMAPeriod=LongMAPeriod, ShortMAPeriod=ShortMAPeriod, MoveStopLoss=10, Order_Quantity=1)

#### 投資組合之平均績效
### 平均總報酬率(複利型)
final_return_MIX = w1*final_return_1 + w2*final_return_2   ##0.5*(final_return_1 + final_return_2)  ##(final_return_1 + final_return_2)*0.5 
